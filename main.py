...
# get the context of tenseal
context = create_tenseal_context()
print(f"\nTenseal context created successfully.")

# Encrypt the input using TenSEAL

# Compile the model for FHE inference.
quantized_module, execution_time = get_quantize_module(
    snn_classifier, torch_input, method="approximate", bits=8
)
...
# Encrypt the input using TenSEAL
tenseal_enc_input = encrypt_with_tenseal(q_input, context)
...
# compare the two encrypted inputs
print(f"\nEncrypted Input (TenSEAL): {tenseal_enc_input}")
print(f"Encrypted Input (FHE): {q_input_enc}")

print(f"Encrypted Input Serialized (TenSEAL): {tenseal_enc_input.serialize()}")
print(f"Encrypted Input Serialized (FHE): {input_buffer}")

print(f"Encrypted Input base64 (TenSEAL): {base64.b64encode(tenseal_enc_input.serialize()).decode('utf-8')}")
print(f"Encrypted Input base64 (FHE): {encrypted_input_str}")
...
tenseal_enc_output = encrypt_with_tenseal(q_y, context)
print(f"Encrypted Output (TenSEAL): {tenseal_enc_output}")
print(f"Encrypted Output (FHE): {q_y_enc}")

print(f"Encrypted Output Serialized (TenSEAL): {tenseal_enc_output.serialize()}")
print(f"Encrypted Output Serialized (FHE): {output_buffer}")

print(
    f"Encrypted Output base64 (TenSEAL): {base64.b64encode(tenseal_enc_output.serialize()).decode('utf-8')}"
)
print(f"Encrypted Output base64 (FHE): {encrypted_output_str}")
